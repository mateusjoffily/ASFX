function TrialInfo = ASFX_ShowTrial(atrial, windowPtr, Stimuli, Cfg)
...
for i = 1:nPages
    ...
    % Show page
    switch Stimuli.type{atrial.pageNumber(i)}
        case 'picture'
            if isfield(Cfg, 'RatingScales') && ...
               ismember(atrial.pageNumber(i), Cfg.RatingScales)
               % Get Verbal Report from rating scale
                [timing(i,:) flipCount(i) Response(i) lastTexture] = ...
                    show_scale(atrial, windowPtr, Stimuli, Cfg, i, lastTexture);

            else
                % Regular picture presentation
                [timing(i,:) flipCount(i) Response(i) lastTexture] = ...
                    show_picture(atrial, windowPtr, Stimuli, Cfg, i);

            end
            
        case 'text' 
            [timing(i,:) flipCount(i) Response(i)] = ...
        ...
        case 'movie'
            [timing(i,:) flipCount(i) lastTexture movie(i)] = ...
                show_movie(atrial, windowPtr, Stimuli, Cfg, i);
...
function [timing flipCount Response stimuliTex]= show_picture(atrial, windowPtr, Stimuli, Cfg, i)

stimuliTex = Stimuli.picture(Stimuli.index(atrial.pageNumber(i))).tex;
...
stimuliTex = [texture ...
              ...
              Stimuli.picture(Stimuli.index(Cfg.RatingScales(end))+1).tex];
...
function [timing flipCount Response] = show_texture(windowPtr, texture, pageDuration, pageNumber, getResponse, Cfg)
...
if strcmp(Cfg.responseDevice, 'MOUSE')
    enter_button_index = 3;   % MOUSE
else
    enter_button_index = 2;   % LUMINA
end
...
while remaining_time > 0
    ...
    if any(buttons) && (t1 - timeLastResp > 0.05)
        if buttons(1) > 0 
            rating.value = 1 + rem(rating.value,5);
            texture(3) = Stimuli.picture(Stimuli.index(Cfg.RatingScales(end))+rating.value+1).tex;
            Screen('DrawTextures', windowPtr, texture, [], destinationRect);
            ASFX_xFlip(windowPtr, texture, Cfg, 1);
            
        elseif buttons(enter_button_index) > 0 && rating.value > 0
            Response.key(1) = rating.value;
