function [nx, ny, textbounds] = ASFX_DrawFormattedText(win, tstring, sx, sy, color, wrapat, flipHorizontal, flipVertical, vSpacing, righttoleft, screenRect)
% [nx, ny, textbounds] = ASFX_DrawFormattedText(win, tstring [, sx][, sy][, color][, wrapat][, flipHorizontal][, flipVertical][, vSpacing][, righttoleft] [, screenRect])
%
...
if nargin < 11 || isempty(screenRect)
...
%     [rect,dh,dv] = CenterRect(bbox, Screen('Rect', win));
    [rect,dh,dv] = CenterRect(bbox, screenRect);
...
%             [rect,dh] = CenterRect(bbox, Screen('Rect', win));
            [rect,dh] = CenterRect(bbox, screenRect);
